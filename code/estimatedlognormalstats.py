...
class EstLogNormalDist:
    ...
    def compute_moments(self):
        ...
        """
        self.mean = math.exp(self.mu + (self.sigma ** 2) * .5)
        self.variance = (math.exp(self.sigma ** 2) - 1) * math.exp(2 * self.mu + (self.sigma)**2)
        self.std_dev = math.sqrt(self.variance)
        self.skewness = (math.exp(self.sigma ** 2) + 2) * (math.sqrt(math.exp(self.sigma ** 2) - 1))
        self.gen_kurtosis = math.exp(4 * self.sigma ** 2) + 2 * math.exp(3 * self.sigma ** 2) + 3 * math.exp(2 * self.sigma ** 2) - 6
    ...
    def graph(self):
        ...
        x = np.linspace(.90, np.max(samples), 1000)
