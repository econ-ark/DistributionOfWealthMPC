...
def get_R_cusp(economy, options):
    economy.agents[0].Ex_PermShkInv = expected(
        lambda x: 1 / x, economy.agents[0].PermShkDstn[0]
    )[0]

    if options["do_lifecycle"]:
        R_cusp_LC = (
            (
                economy.agents[0].PermGroFac[0]
                / (economy.agents[0].Ex_PermShkInv * economy.agents[0].LivPrb[0])
            )
            ** economy.agents[0].CRRA
        ) * (1 / economy.agents[0].DiscFac)
        return R_cusp_LC
    else:
        R_cusp_PY = (
            (economy.agents[0].PermGroFac[0] / economy.agents[0].Ex_PermShkInv)
            ** economy.agents[0].CRRA
        ) * (1 / economy.agents[0].DiscFac)
        return R_cusp_PY
...
def estimate_r_lower(options, params):
    ...
    R_cusp = get_R_cusp(economy, options)
    epsilon = 1e-8
    ...
    if options["run_estimation"]:
        ...
        if options["param_name"] == "Rfree":
            ...
            param_range = [
                economy.agents[0].Rfree - 0.01,
                economy.agents[0].Rfree + 0.1,
            ]
        else:
...
def estimate_r_upper_given_r_lower(options, params):
    ...
    if options["run_estimation"]:
        print(
            f"Beginning an estimation with the specification name {economy.spec_name}..."
        )
