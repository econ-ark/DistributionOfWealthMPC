...
T_to_use = 200
C_data_big = np.concatenate(
    [EstimationEconomy.agents[j].history["cNrm"][-T_to_use:, :] for j in range(7)]
).flatten()
Y_data_big = np.concatenate(
    [EstimationEconomy.agents[j].history["yNrm"][-T_to_use:, :] for j in range(7)]
).flatten()
MPC_data_big = np.concatenate(
    [EstimationEconomy.agents[j].history["MPC"][-T_to_use:, :] for j in range(7)]
).flatten()
age_data_big = np.concatenate(
    [EstimationEconomy.agents[j].history["t_age"][-T_to_use:, :] for j in range(7)]
).flatten()
